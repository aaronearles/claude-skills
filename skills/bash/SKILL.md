...
name: bash
description: >
  Organizational standards and best practices for Bash script development.
  Use this skill whenever writing, reviewing, scaffolding, or refactoring any Bash or
  shell scripts — including .sh files, script headers, strict mode, error handling,
  input validation, trap/cleanup, security practices, shellcheck compliance, or bats testing.
  Also use when creating new shell scripts, reviewing for standards compliance, or
  generating boilerplate/templates. Always check for Windows CRLF line endings on AI-generated code.
---

# Bash Development Standards

This document outlines the standards and best practices for Bash script development.
...
- **CRITICAL**: Ensure Unix line endings (LF) - check for Windows CRLF that breaks shebang execution
...
- **WARNING**: AI-generated code often has Windows CRLF line endings - always run `dos2unix` after copy/paste
...
```bash
# Check for Windows line endings (CRLF) that break bash execution
file scripts/*.sh | grep -i crlf
# or
head -1 script.sh | od -c  # Look for \r\n instead of \n
